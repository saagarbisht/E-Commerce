...
const PurchaseSuccessPage = () => {
  ...
  useEffect(() => {
    const handleCheckoutSucccess = async (sessionId) => { 
      try {
    ...
    if(sessionId){
      handleCheckoutSucccess(sessionId)
    }else{
      setIsProcessing(false)
    }
  },[clearCart])
  ...
  return (
    <div className="h-screen flex items-center justify-center px-4">
      <Confetti width={window.innerWidth} height={window.innerHeight} gravity={0.1} style={{zIndex:'999'}} numberOfPieces={800} recycle={false}/>
